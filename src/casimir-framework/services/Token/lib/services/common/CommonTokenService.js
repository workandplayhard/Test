import { AssetType, APP_PROPOSAL } from '@/casimir-framework/vars';
import { TransferFTCmd, TransferNFTCmd } from '@/casimir-framework/commands';
...
export class CommonTokenService {
  ...
  createTokensSwapProposal(payload) {
    const swapPayload = updateProposalInfo(payload, {
      isProposal: true
    });
    const {
      data: {
    ...
    return transferToken(swapPayload, transferCommands, APP_PROPOSAL.TOKENS_SWAP_PROPOSAL, this.tokenHttp.createTokensSwapProposal);
  }
